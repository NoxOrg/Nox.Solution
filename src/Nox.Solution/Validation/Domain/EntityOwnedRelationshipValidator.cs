...
namespace Nox
{
    internal class EntityOwnedRelationshipValidator: AbstractValidator<EntityOwnedRelationship>
    {
        ...
        public EntityOwnedRelationshipValidator(string entityName, IEnumerable<EntityOwnedRelationship>? relationships, IEnumerable<Entity>? entities)
        {
            ...
            RuleFor(er => er.Relationship)
                .NotEmpty()
                .WithMessage(er => string.Format(ValidationResources.EntityOwnedRelationshipRelationshipEmpty, er.Name, entityName, EntityRelationshipTypeHelpers.NameList()));
